		...
		<div class="container-fluid" id="icerik">
			<div class="row">
				...
				<div class="col-6" style="margin-top: 10px;" id="orta">
					...
					<?php 
					$db->sql = "select * from users where user_id=? and confirmed=?";
					$db->veri = array($id,1);
					$userbilgi = $db->select();
					...
					$db->sql = "select * from news INNER JOIN users ON news.user_id=users.user_id where news.user_id=? and news.confirmed = ? ORDER BY post_id DESC";
					$db->veri = array($id,1);
					$etkinlik = $db->select();
					if ($etkinlik == false) {
						?>
