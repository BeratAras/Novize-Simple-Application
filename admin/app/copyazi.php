...
<div id="wrapper">
 ...
 <div id="page-wrapper">
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">Copyright Yazısını Güncelle</h1>
        <!-- /.row -->
        <div class="row">
          <div class="col-lg-6">
            <?php
            if (isset($_POST["guncelle1"])) {
              $copyazi = post("copyazi");
   
                  $db->sql = "update site_texts set text=?, date=? where text_type=?";
                  $db->veri = array($copyazi, date("d, m, y"), "CopYazi");
                  $guncelle = $db->update();
                  if($guncelle){
                  ...
                  <div class="col-lg-6">
                    ...
                    <form role="form" method="POST"> <!-- Yazı Post-Form Kısmı -->
                      <div class="form-group ckeditor">
                        ...
                        $copyazi = $db->select();
                        foreach ($copyazi as $key => $value) {
                         ?>
                        <label>Copyright:</label>
                        <textarea name="copyazi" rows="10" class="form-control"><?=$value->text;?></textarea>
                      </div>
                        <?php 
                      }
                       ?>
