...
<div id="wrapper">
 ...
 <div id="page-wrapper">
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">Site Kurallarını Güncelle</h1>
        <!-- /.row -->
        <div class="row">
          <div class="col-lg-6">
            <?php
            if (isset($_POST["guncelle1"])) {
              $sitekural = post("sitekural");
   
                  $db->sql = "update site_texts set text=?, date=? where text_type=?";
                  $db->veri = array($sitekural, date("d, m, y"), "SiteKural");
                  $guncelle = $db->update();
                  if($guncelle){
                  ...
                  <div class="col-lg-6">
                    ...
                    <form role="form" method="POST"> <!-- Yazı Post-Form Kısmı -->
                      <div class="form-group ckeditor">
                        ...
                        $sitekural = $db->select();
                        foreach ($sitekural as $key => $value) {
                         ?>
                        <label>Site Kuralı Gir:</label>
                        <textarea name="sitekural" rows="10" class="form-control ckeditor"><?=$value->text;?></textarea>
                      </div>
                        <?php 
                      }
                       ?>
